...
describe('Rock, Paper, Scissors', function () {
  ...
  describe('makeGame', function () {
    ...
    it('Should revert on entryFee below minimum', async function () {
      ...
      await expect(rps.connect(p1).makeGame(hashedChoice, { value: weiAmount })).to.be.revertedWith(
        ERRORS.AmountTooLow,
      );
    });
  });
  ...
  describe('joinGame', function () {
    ...
    it('Should revert on index out of bounds p2', async function () {
      ...
      await expect(
        rps.connect(p2).joinGame(p1.address, gameIndex, p2Choice, { value: entryFee }),
    ...
    it('Should revert on player joining his own game', async function () {
      ...
      await expect(
        rps.connect(p1).joinGame(p1.address, gameIndex, p1Choice, { value: entryFee }),
    ...
    it('Should revert if game already has a second player', async function () {
      ...
      await rps.connect(p2).joinGame(p1.address, gameIndex, p2Choice, { value: entryFee });
      await expect(
        rps.connect(p3).joinGame(p1.address, gameIndex, p3Choice, { value: entryFee }),
      ).to.be.revertedWith(ERRORS.CannotJoinGame);
    });
  });
  ...
  describe('resolveGameP1', function () {
    it('Should revert on index out of bounds', async function () {
      ...
      await expect(rps.connect(p1).resolveGameP1(oufOfBoundsIndex, clearChoice)).to.be.revertedWith(
    ...
    it("Shouldn't let p1 resolve the game if doesn't have one", async function () {
      ...
      await expect(rps.connect(p1).resolveGameP1(gameIndex, gameClearChoice)).to.be.revertedWith(
        ERRORS.IndexOutOfBounds,
      );
    });
  });
  ...
  describe('resolveGameP2', function () {
    it("Shouldn't let p2 resolve the game if timer is still running", async function () {
      ...
      await expect(rps.connect(p2).resolveGameP2(p1.address, gameIndex)).to.revertedWith(ERRORS.TimerStillRunning);
    });
    ...
    it("Shouldn't let p2 resolve the game if game doesn't have a second player", async function () {
      ...
      await expect(rps.connect(p2).resolveGameP2(p1.address, gameIndex)).to.revertedWith(
        ERRORS.NoSecondPlayer,
      );
    });
    ...
    it('Should clean up after resolve', async function () {
      ...
      await expect(rps.connect(p1).getGame(p1.address, gameIndex)).to.be.revertedWith(
        ERRORS.IndexOutOfBounds,
      );
    });
